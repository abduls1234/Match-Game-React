...
class App extends Component {
  ...
  componentDidMount() {
    this.setTimerIntervalId = setInterval(this.timer, 1000)
  }
  ...
  clearIntervalId = () => clearInterval(this.setTimerIntervalId)

  stateChange = () => {
    this.setState(prevState => ({isGameStarted: !prevState.isGameStarted}))

    this.setState({score: 0, time: 60, activeImage: imagesList[0]})
  }

  WinOrLose = () => {
    const {score} = this.state
    return <WinOrLoseCard score={score} playAgain={this.stateChange} />
  }
  ...
  clickImage = val => {
    ...
    if (id === val) {
      ...
      this.setState({
        activeImage: imagesList[Math.floor(Math.random() * imagesList.length)],
      })
    } else {
      this.setState(prevState => ({isGameStarted: !prevState.isGameStarted}))
  ...
  render() {
    const {score, isGameStarted, time} = this.state

    return (
      <div className="app-container">
        <Header scoreDetails={score} timeDetails={time} />
        <div className="match-game-container">
          <div className="images-card">
            {isGameStarted ? this.renderImagesList() : this.WinOrLose()}
          </div>
        </div>
      </div>
    )
